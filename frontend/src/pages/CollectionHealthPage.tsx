...
export default function CollectionHealthPage() {
  ...
  const [tab, setTab] = useState<'dates' | 'errors'>('dates');
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setErr('');
    const p = tab === 'dates' ? api.instanceCollectionDates(id) : api.instanceCollectionErrors(id, days);
    p.then((r) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center gap-3">
        <CalendarClock className={clsx('w-7 h-7', isDesktopData ? 'text-[#0078d4]' : 'text-amber-400')} />
        <div>
          <h1 className="text-2xl font-semibold text-white">Collection health</h1>
          <p className="text-sm text-gray-400">CollectionDates_Get — CollectionErrorLog_Get</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-end">
        ...
        <TabNav
          tabs={[
            { key: 'dates', label: 'Collection dates' },
            { key: 'errors', label: 'Errors' },
          ]}
          active={tab}
          onChange={(k) => setTab(k as 'dates' | 'errors')}
        />
        {tab === 'errors' && (
          <div>
            ...
            <input
              ...
              className="mt-1 w-24 rounded-md bg-white/5 border border-white/10 px-3 py-2 text-sm text-white"
            />
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className={clsx('rounded-lg border p-4', dataGridShellClass)}>
          <DataTable columns={columns} data={rows} exportCsvFileName="collection-health" />
        </div>
      ) : id !== '' ? (
