...
export default function LogShippingPage() {
  ...
  useEffect(() => {
    ...
    setErr('');
    const p =
      tab === 'summary' ? api.instanceLogShippingSummary(id) : api.instanceLogShippingDetail(id);
    p.then((r) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center gap-3">
        <Ship className={clsx('w-7 h-7', isDesktopData ? 'text-[#0078d4]' : 'text-cyan-400')} />
        <div>
          <h1 className="text-2xl font-semibold text-white">Log shipping</h1>
          <p className="text-sm text-gray-400">Same data as DBA Dash WinForms — dbo.LogShippingSummary_Get / LogShipping_Get</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className={clsx('rounded-lg border p-4', dataGridShellClass)}>
          <DataTable columns={columns} data={rows} exportCsvFileName="log-shipping" />
        </div>
      ) : id !== '' ? (
