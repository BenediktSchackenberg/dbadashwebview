...
export default function EstateLogShippingPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Estate — log shipping</h1>
          <p className="text-sm text-gray-400">dbo.LogShipping_Get (all instances)</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className={clsx('rounded-lg border p-4', dataGridShellClass)}>
          <DataTable columns={columns} data={rows} exportCsvFileName="estate-log-shipping" />
        </div>
      ) : (
