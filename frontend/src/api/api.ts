...
export const api = {
  ...
  dashboardSummary: () => request<ApiRow[]>('/api/dashboard/summary'),
  dashboardStats: () => request<DashboardStats>('/api/dashboard/stats'),
  instances: () => request<SearchInstanceRow[]>('/api/instances'),
  instance: (id: number) => request<any>(`/api/instances/${id}`),
  instanceCpu: (id: number, hours = 24) => request<ApiRow[]>(`/api/instances/${id}/cpu?hours=${hours}`),
  instanceWaits: (id: number, hours = 24) => request<ApiRow[]>(`/api/instances/${id}/waits?hours=${hours}`),
  instanceDrives: (id: number) => request<ApiRow[]>(`/api/instances/${id}/drives`),
  instanceDatabases: (id: number) => request<ApiRow[]>(`/api/instances/${id}/databases`),
  instanceBackups: (id: number) => request<ApiRow[]>(`/api/instances/${id}/backups`),
  instanceJobs: (id: number) => request<ApiRow[]>(`/api/instances/${id}/jobs`),
  jobsRecent: () => request<SearchJobRow[]>('/api/jobs/recent'),
  ...
  dashboardPerformanceSummary: () =>
    request<{ data: ApiRow[]; note: string }>('/api/dashboard/performance-summary'),
  ...
  dashboardMonitor: () => request<{ instances: any[]; alertCounts: Record<string, number>; recentErrors: ApiRow[] }>('/api/dashboard/monitor'),
  getThresholds: () =>
